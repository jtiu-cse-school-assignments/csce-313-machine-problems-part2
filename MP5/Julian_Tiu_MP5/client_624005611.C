...
void *eventHandlerFunction(void *arg) {
	...
	//NetworkRequestChannel chan(parameters->hostName, parameters->_port_no);
	//RequestChannel chan("control", RequestChannel::CLIENT_SIDE);
	
	NetworkRequestChannel ** RCA = new NetworkRequestChannel*[parameters->numberOfRequests];
	...
	for(int i = 0; i < numRequestChannels; i++) {
		//string newThread = chan.cwrite("newthread");
   	 	//RCA[i] = new RequestChannel(newThread, RequestChannel::CLIENT_SIDE);
   	 	RCA[i] = new NetworkRequestChannel(parameters->hostName, parameters->_port_no);
   	 	cout << "Connection " << i << " Established" << endl;
	}
    ...
    for(int i = 0; i < parameters->numberOfRequests; i++) {
 		FD_SET(RCA[i]->socketfd, &readSet);
 		if(RCA[i]->socketfd > max) {
 			max = RCA[i]->socketfd;
 		}
 		string req = parameters->bb->fromBuffer();
		string name;
		if(req.size() > 5) name = req.substr(5);
        fd_to_id.insert(pair<int,string>(RCA[i]->socketfd, name));
		RCA[i]->cwrite(req);
 	}
	...
	while(foreverLoop) {
 		...
 		for(int i = 0; i < parameters->numberOfRequests; i++) {
 			if(FD_ISSET(RCA[i]->socketfd, &readSet)) {
 				ready = RCA[i]->socketfd;		
...
int main(int argc, char * argv[]) {
	...
	while( (c = getopt(argc, argv, "b:w:n:h:p:")) != -1) {
		switch(c) {
			case 'b':
		...
		eventArguments->_port_no = port_no;
		eventArguments->hostName = hostname;
